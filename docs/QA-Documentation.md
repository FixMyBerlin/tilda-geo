...
1. **Reference data**: Stored in a dated `data.*` baseline (currently `data.euvm_qa_voronoi_2026`); during processing copied to `public.qa_parkings_euvm` / `public.qa_parkings_euvm_priority` (recreatable with limitations; all public). Script: [`parking/9_qa_parkings_euvm_voronoi.sql`](../processing/topics/parking/9_qa_parkings_euvm_voronoi.sql). The script keeps previous run data for comparison and joins TILDA data to reference; update rules use it ([§3.1](#31-system-overwrites-system-no-user-decision), [§3.2](#32-system-overwrites-user-decision)). Non-production environments import the real baseline via `/admin/data-schema` instead of relying on the empty placeholder that step 9 creates when the table is missing. Publish the undated base table `data.euvm_qa_voronoi` once so the generator can be re-run outside production.
