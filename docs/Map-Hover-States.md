...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| List hover → map marker    | `ModeListItem` / `ModeDataTableRow` write `{ id, coordinates }` to `mode-list-store`. Off-screen items use `MapListHoverMarker` (clamped to the canvas edge). In-view **notes** paint a fill-only `${layerId}-hover` disc under the icon (`noteHoverCirclePaint`); QA and Prüflisten still use the HTML ring on the point. `ModePanel` shows “Außerhalb des Kartenausschnitts” while `atEdge`. | Live in all three modes |
...
List hover no longer merges the hovered id into those `*-selected` filters. Notes in-view list hover filters `${layerId}-hover` by the GeoJSON `id` property. Off-screen notes and other modes still use the centroid HTML marker.
