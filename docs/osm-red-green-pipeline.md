...
| Staging build | `stagingBuild.ts` | `withProcessEnv(dbEnv(staging))` points `psql`/`params` at the staging DB, then runs the normal `processTopics()` full rebuild there. The live primary is never written during the build. |
| Validate | `validateStaging.ts`, `promotableTables.ts` | Discover promotable `public.*` `BASE TABLE`s (excludes `*_diff`, honors `PROMOTION_TABLES_ALLOWLIST`). Require `meta` rows, reject zero-row **non-intermediate** tables (intermediate = name starting with `_`), and require each table's staging row count to be within `PROMOTION_ROW_COUNT_TOLERANCE` of primary. |
| Promote | `promotePrimary.ts` | Via `postgres_fdw`: create a `geo_stage_server_<runId>` server + user mapping to staging, `IMPORT FOREIGN SCHEMA` into `geo_stage_import`, copy each table into `geo_shadow` (per-table, **outside** a transaction), then **one** transaction swaps `public.*` → `geo_prev.*` and `geo_shadow.*` → `public.*`, then drops the import schema + FDW server. The denylist blocks app/user tables — a fixed set (`User, Account, Session, Verification, Region, Membership, Upload, Note, QaConfig, Processing`) plus the `prisma_*` pattern. |
...
| Gate / rollback | `verifier.ts`, `rollbackPrimary.ts` | At the end, if the verifier's max failure streak exceeded `RED_GREEN_MAX_FAILURE_STREAK`, fail the run and `rollbackPrimary` swaps `geo_prev` back into `public`. The lock is always released in `finally`. |
...
`generate-tiles.production.yml` and `generate-tiles.staging.yml` both run every
3 hours (`0 */3 * * *`) with `PIPELINE: red-green`. The reusable
...
### `processing` is an on-demand worker, not a service

Do **not** `docker compose up -d processing`. The container only runs per-invocation
via `docker compose run --rm processing …`. `deploy-processing.yml` therefore
only *pulls* the new image on deploy — it deliberately does not start the
container, which previously launched the legacy in-place `index.ts` against the
primary DB on every deploy and raced the red/green run.
...
deliberately *approximate at borders* between reseeds — acceptable because the
...
The per-table FDW copy loop runs statement-by-statement *outside* a transaction
...
Geofabrik *internal* OAuth baseline with full metadata).
...
1. **Replication source.** Planet hourly carries full metadata but applies
   *global* diffs to a Germany file (bloat until the nightly reseed). The OSM-fr
...
2. **FDW full-copy cost.** Promotion copies every promoted table's full contents
   through `postgres_fdw` and rebuilds indexes on primary *every run*. At Germany
