...
Masks are created using the `nub run regions:masks` script, which:
...
- `nub run regions` - Runs both `regions:masks` and `regions:configs`
- `nub run regions:masks` - Creates/updates regional masks
- `nub run regions:configs` - Updates category configs
