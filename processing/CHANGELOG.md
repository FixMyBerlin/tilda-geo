...
- **Geometry direction** of derived bikelanes: `…/right` runs in the **OSM way direction**. `…/left` (`offset` > 0) runs against the OSM way, in right-hand-traffic flow. Direction-relative attributes (`mapillary_forward`/`mapillary_backward`, `traffic_sign_forward`/`traffic_sign_backward`) stay relative to the OSM way direction, so on `…/left` lanes they point opposite the exported geometry.
