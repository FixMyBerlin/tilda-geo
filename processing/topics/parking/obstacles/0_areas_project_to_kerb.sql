...
--     project to regular kerbs using `project_to_k_closest_kerbs` (finds up to 6 closest kerbs)
--   * If obstacle DOES lie within separate parking area:
--     project to separate parking area kerbs using `project_to_line` (direct projection)
...
-- If obstacle does NOT lie within separate parking area:
-- Project to regular kerbs using `project_to_k_closest_kerbs` (finds up to 6 closest kerbs within 2m)
...
-- If obstacle DOES lie within separate parking area:
-- Project directly onto separate parking area kerb line using `project_to_line`.
-- The kerb line is created via `parking_area_to_line` in `_parking_separate_parking_areas_projected`.
INSERT INTO
...
-- `project_to_k_closest_kerbs` can return up to 6 projections (k := 6) for each obstacle area.
