...
--     project to regular kerbs using `project_to_k_closest_kerbs` (finds closest kerb)
...
-- If obstacle does NOT lie within separate parking area:
-- Project to regular kerbs using `project_to_k_closest_kerbs` (finds closest kerb within 5m)
...
-- `project_to_k_closest_kerbs` can return multiple projections (k := 1, but may still have duplicates).
