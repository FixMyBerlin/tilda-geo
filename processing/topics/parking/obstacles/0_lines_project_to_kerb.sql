...
-- When an obstacle line has multiple projections to the same kerb, keep only the closest one (remove furthest).
...
DO $$ BEGIN RAISE NOTICE 'END projecting obstacle lines at %', clock_timestamp() AT TIME ZONE 'Europe/Berlin'; END $$;
