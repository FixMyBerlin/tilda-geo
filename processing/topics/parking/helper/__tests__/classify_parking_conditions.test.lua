...
describe('classify_parking_conditions', function()
  ...
  it('returns condition_category for paid parking', function()
    ...
    local result = classify_parking_conditions(tags, nil, 'assumed_free')
  ...
  it('uses parent_tags when value is only on parent (e.g. road maxweight)', function()
    local tags = {}
    local parent_tags = { fee = 'yes' }
    local result = classify_parking_conditions(tags, parent_tags, 'assumed_free')
    assert.are.equal(result.condition_category, 'paid')
  end)

  it('uses default_category when no conditions apply', function()
    local result = classify_parking_conditions({}, nil, 'assumed_free')
    assert.are.equal(result.condition_category, 'assumed_free')
    local result_private = classify_parking_conditions({}, nil, 'assumed_private')
