...
function parking_roads(object)
  ...
  local row = MergeTable({ geom = object:as_linestring(), is_driveway = is_driveway, has_parking = has_parking(object.tags) }, result_tags_roads(object))
  local cleaned_tags, replaced_tags = sanitize_cleaner(row.tags, object.tags)
