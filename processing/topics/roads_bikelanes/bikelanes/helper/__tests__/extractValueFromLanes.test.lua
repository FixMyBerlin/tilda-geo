...
describe('extractValueFromLanes helper', function()
  ...
  describe('extractLastValueFromLanes', function()
    it('should extract the last value from lanes schema', function()
      local result = extractValueFromLanes.extractLastValueFromLanes('1|2|3')
      assert.are.equal(result, '3')
    end)

    it('should handle single value', function()
      local result = extractValueFromLanes.extractLastValueFromLanes('single')
      assert.are.equal(result, 'single')
    end)

    it('should handle empty values in lanes schema', function()
      local result = extractValueFromLanes.extractLastValueFromLanes('1||3')
      assert.are.equal(result, '3')
    end)

    it('should handle empty string', function()
      local result = extractValueFromLanes.extractLastValueFromLanes('')
      assert.are.equal(result, '')
    end)

    it('should handle nil input', function()
      local result = extractValueFromLanes.extractLastValueFromLanes(nil)
      assert.are.equal(result, nil)
    end)

    it('should handle complex lanes schema', function()
      local result = extractValueFromLanes.extractLastValueFromLanes('asphalt|concrete|gravel|dirt')
