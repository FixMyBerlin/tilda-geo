...
export async function promoteStagingIntoPrimary(
	...
	await executeSql(
		config.primary,
		"CREATE SCHEMA IF NOT EXISTS geo_stage_import",
	);
	...
	await executeSql(
		config.primary,
		`DROP SERVER IF EXISTS ${qIdent(fdwServer)} CASCADE`,
	);
	await executeSql(
		config.primary,
	...
	for (const table of tables) {
		...
		await executeSql(
			config.primary,
			`DROP TABLE IF EXISTS geo_shadow.${ident} CASCADE`,
		);
	...
	await executeSql(
		config.primary,
		`BEGIN; ${swapStatements.join("; ")}; COMMIT;`,
	);
	await executeSql(
		config.primary,
		"DROP SCHEMA IF EXISTS geo_stage_import CASCADE",
