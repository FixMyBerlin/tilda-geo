...
function parseDbUrl(envName: string): ParsedDbUrl {
  ...
  if (parsed.protocol !== 'postgresql:' && parsed.protocol !== 'postgres:') {
    throw new Error(`Invalid protocol for ${envName}. Expected postgres:// or postgresql://`)
  }
...
export function readRedGreenConfig(): RedGreenConfig {
  const warmCacheMode = process.env.WARM_CACHE_MODE === 'delta' ? 'delta' : 'full'
  const tablesAllowlist = parseCsv('PROMOTION_TABLES_ALLOWLIST')

  const rowCountTolerance = Number(process.env.PROMOTION_ROW_COUNT_TOLERANCE || '0.15')
  ...
  return {
    workingPbfPath: process.env.WORKING_PBF_PATH || '/data/downloads/germany-latest.osm.pbf',
    recutPbfPath: process.env.RECUT_PBF_PATH || '/data/filtered/germany-recut.osm.pbf',
    ...
    replicationLoopSleepSeconds: Number(process.env.REPLICATION_LOOP_SLEEP_SECONDS || '60'),
    rowCountTolerance,
    warmCacheMode,
