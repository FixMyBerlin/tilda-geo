...
export async function downloadBaselineIfNeeded(config: RedGreenConfig) {
	if (!(await shouldReseedBaseline(config.workingPbfPath))) {
		console.log(
			`Baseline: keeping ${config.workingPbfPath} (intra-day pyosmium catch-up)`,
		);
		return;
	}
	...
	console.log(
		`Baseline: reseed complete (${basename(targetPath)}, marker ${today})`,
	);
}
...
export async function ensureRecutPolygon(config: RedGreenConfig) {
	...
	console.log(
		`Polygon: downloading clip polygon from ${config.recutPolygonDownloadUrl}`,
	);
	await runLogged(
		`wget -q -O "${config.recutPolygonPath}" "${config.recutPolygonDownloadUrl}"`,
	);
