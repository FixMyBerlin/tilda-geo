...
async function writeReplicationCheckpoint(config: RedGreenConfig) {
	try {
		...
		const checkpoint = join(
			HASH_DIR,
			`red_green_replication_${config.runId}.json`,
		);
		await writeFile(
			checkpoint,
			`${JSON.stringify(
...
export async function replicateAndRecut(config: RedGreenConfig) {
	...
	while (status === 1) {
		...
		if (status === 1) {
			...
			throw new Error(
				`pyosmium-up-to-date failed with exit code ${status} on attempt ${attempts}`,
			);
		}
	}
	...
	throw new Error(
		"Missing recut definition. Set either RECUT_POLYGON_PATH or RECUT_BBOX.",
	);
}
