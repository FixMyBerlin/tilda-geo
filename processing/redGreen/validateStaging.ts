...
import {
	assertPromotableTables,
	isIntermediateTable,
} from "./promotableTables";
...
export async function validateStagingDataset(
	config: RedGreenConfig,
	...
	if (tables.length === 0) {
		throw new Error(
			"Validation failed: no promotable public tables found in staging DB.",
		);
	}
	...
	if (metaRows === null || metaRows <= 0) {
		throw new Error(
			"Validation failed: public.meta has no rows in staging DB.",
		);
	}
	...
	for (const table of tables) {
		...
		if (stagingCount === null) {
			throw new Error(
				`Validation failed: unable to count rows for public.${table}`,
			);
		}

		if (stagingCount === 0 && !isIntermediateTable(table)) {
			throw new Error(
				`Validation failed: public.${table} has zero rows in staging DB.`,
			);
		}
		...
		if (primaryCount === null) {
			throw new Error(
				`Validation failed: unable to count rows for public.${table} on primary`,
			);
		}

		assertRowCountWithinTolerance(
			table,
			stagingCount,
			primaryCount,
			config.rowCountTolerance,
		);
	}
