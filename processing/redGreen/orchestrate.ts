...
import { downloadBaselineIfNeeded, ensureRecutPolygon } from './downloadBaseline'
...
async function main() {
  ...
  if (endpoints.length === 0) {
    throw new Error('RED_GREEN_VERIFY_ENDPOINTS must be set. Verification is mandatory.')
  }
  ...
  try {
    ...
    // Only tables whose swap transaction committed may be rolled back. Before the swap,
    // public.* is untouched and a rollback would drop live data (geo_prev may be empty).
    promotedTables = await promoteStagingIntoPrimary(config, validation.tables)
    await checkEndpoints()
    ...
    if (params.skipTileServerRestart) {
      console.log('Finishing up: ⏩ Skipping tile server restart due to SKIP_TILES_RESTART=1')
    } else {
      try {
    ...
    const maxFailureStreak = Number(process.env.RED_GREEN_MAX_FAILURE_STREAK || '3')
    ...
    if (promotedTables.length > 0) {
      try {
        ...
        console.error(`[ERROR] Rollback failed for run ${config.runId}`, rollbackError)
      }
    }
    await verifier.stop()
    throw error
