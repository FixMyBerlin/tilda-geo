...
/** @desc Takes the layers we extract from Mapbox with `nub run mapbox-styles-update` (which are stripped down to just the style information) and adds the source-information that is only present in our app. It also allows to use the same layers with differend `additionalFilter`.  */
