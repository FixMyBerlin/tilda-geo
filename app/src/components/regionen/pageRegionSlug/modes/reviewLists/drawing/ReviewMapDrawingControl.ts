...
type Options = {
  ...
  onGeometryFinish: (geometry: GeoJSON.Geometry) => Promise<unknown> | void
  /**
  ...
  onSelectionChange: (selectedCount: number, partCount: number) => void
}
...
export class ReviewMapDrawingControl {
  ...
  private loadedEditEntryId: number | undefined
  ...
  onRemove(map: MapLibreMap) {
    ...
    this.selectedIds = []
    this.loadedEditEntryId = undefined
  ...
  loadEditFeature(
    ...
    if (!options?.force && this.loadedEditEntryId === entryId) return true
    ...
    this.loadedEditEntryId = entryId
  ...
  private persistSnapshotGeometry(id?: string | number) {
    ...
    if (this.options.isCreateSession()) {
      const feature =
        id === undefined ? this.draw.getSnapshot()[0] : this.draw.getSnapshotFeature(id)
      if (!feature) return
      this.tryFinishGeometry(feature.geometry as GeoJSON.Geometry)
      return
    }
    this.persistEditSnapshot()
  }
  ...
  private persistEditSnapshot() {
    ...
    if (!this.tryFinishGeometry(combined)) return
    this.loadedGeometry = combined
  }

  private tryFinishGeometry(geometry: GeoJSON.Geometry) {
    if (this.persistInFlight) return false
    this.persistInFlight = true
    try {
      const result = this.options.onGeometryFinish(geometry)
      if (result) {
        void result
          .finally(() => {
            this.persistInFlight = false
  ...
  private notifySelectionChange() {
    this.options.onSelectionChange(this.getSelectedIds().length, this.getPartCount())
  }
  ...
  private attachListeners() {
    ...
    this.draw.on('deselect', (id) => {
      ...
      this.notifySelectionChange()
      if (this.options.isCreateSession()) return
