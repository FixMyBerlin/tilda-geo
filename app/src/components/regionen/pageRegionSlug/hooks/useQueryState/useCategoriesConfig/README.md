...
Migrations for regions (which are `params.regionSlug`) happen inside `server/regions/getRegionRedirectUrl.ts`.
...
Whenever we change the `categories.const.ts`, the test [`checksums.test.ts`](./v2/tests/checksums.test.ts) will fail and remind us to run `bun run regions:configs` to generate and store a new config template for each region.
