...
import type { FileMapDataSubcategoryStyleLayer } from '../../../../mapData/types'
...
describe('mergeCategoriesConfig()', () => {
  ...
  test('`active` states are re-applied (if `id` stayed the same)', () => {
    ...
    const style0 = urlConfig[0]?.subcategories[0]?.styles[0]
    const style1 = urlConfig[0]?.subcategories[0]?.styles[1]
    if (style0) style0.active = false // subcategory:surfaceRoads style-id:default
    if (style1) style1.active = true // subcategory:surfaceRoads style-id:completenes
  ...
  test('Renamed category IDs are ignored and fall back to defaults', () => {
    ...
    // This is why we need migrateConfigCategoryIds() in getRegionRedirectUrl to map old IDs to new ones
