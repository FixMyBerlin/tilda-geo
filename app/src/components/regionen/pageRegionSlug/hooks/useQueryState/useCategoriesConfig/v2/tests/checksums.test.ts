...
describe('Check if configs were changed', () => {
  test('Validate that the current config is stored. Run `nub run regions:configs` if this fails.', () => {
    // This test fails if the config was changed.
    // Which means you need to run `nub run regions:configs` to preserve this config as a template.
