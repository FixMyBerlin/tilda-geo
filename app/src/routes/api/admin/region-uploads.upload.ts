...
const router: Router = {
  ...
  routes: {
    regionLogo: route({
      ...
      // the file alone also does not say which region the logo belongs to.
      ...
      onBeforeUpload: async ({ req, clientMetadata, file }) => {
        ...
        if (region.slug !== clientMetadata.regionSlug) {
          throw new Error('Region-Slug stimmt nicht mit der Region-ID überein')
        }
      ...
      onAfterSignedUrl: async ({ file, metadata }) => {
        ...
        // Server → client (onUploadComplete): so the form can set headerLogoId.
        return {
