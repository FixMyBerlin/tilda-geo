...
describe('resolveLatestDataSchemaDumpKey', () => {
  ...
  it('prefers the content-addressed object dump when it exists', async () => {
    ...
    await expect(resolveLatestDataSchemaDumpKey('euvm_cutouts_point', sha256)).resolves.toBe(
  ...
  it('falls back to latest/table.dump for non-hex sha256 without calling S3', async () => {
    await expect(resolveLatestDataSchemaDumpKey('euvm_cutouts_point', 'abc')).resolves.toBe(
      'data-schema/euvm_cutouts_point/latest/table.dump',
    )
