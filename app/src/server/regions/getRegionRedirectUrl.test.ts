...
import { getRegionRedirectUrl } from './getRegionRedirectUrl'
...
describe('getRegionRedirectUrl()', () => {
  test('handles path-only URLs (e.g. from TanStack Router location.href)', () => {
    const pathOnlyUrl = '/regionen/parkraum?map=13.5/52.4918/13.4261'
    const redirectUrl = getRegionRedirectUrl(pathOnlyUrl, 'parkraum')
  ...
  test('redirect preserves request host', () => {
  ...
  describe('Make sure the redirects work for /regionen/:slug', () => {
    ...
    test('MIGRATION: Update old `config`s: Check if all `nameMigrations` are done', () => {
      const redirectUrl = getRegionRedirectUrl(
    ...
    test('MIGRATION: Make sure params are only present once', () => {
      const redirectUrlMigrated = getRegionRedirectUrl(
        sharedMockMigrationUrl,
        extractSlugFromUrl(sharedMockMigrationUrl),
      )
    ...
    test('TEST: Invalid map param is handled properly', () => {
      ...
      let redirectUrl = getRegionRedirectUrl(url, extractSlugFromUrl(url))
      expect(redirectUrl).toBeTruthy()
      ...
      redirectUrl = getRegionRedirectUrl(url, extractSlugFromUrl(url))
      expect(redirectUrl).toBeTruthy()
