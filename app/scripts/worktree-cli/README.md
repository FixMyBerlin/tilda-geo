...
- **Ports:** the app dev server is fixed to port 4000 (`strictPort`). Only one worktree can run `bun run dev` at a time on the same machine; otherwise, auth will not work.
